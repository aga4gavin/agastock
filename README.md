...
## 開發動機
這是2021年長榮套在202元，決心寫程式脫魯所產出的成果，話不多說有圖有真相(哭)      
<img src="./doc/2603.png" style="zoom:35%" />   

話說買股票這些年來，很神奇的每次追高就大跌殺低就大漲，據說這就是魯蛇平均值。曾上過金融研訓院的量化交易課，老師說好策略不用程式也能賺錢，爛策略走程式賠更快，嚇死了這不可能是我。

先說結論，經過三個月數個不出門的周末，產出個簡單的網站，先別笑很簡陋，這可是C語言工程嘗試寫 Python，歡迎大家給予寶貴的建議！

## 簡介
對設定好的台股美股清單，定期下載股票資訊，計算布林通道，Google Trend 等指標，取得 EPS 及本益比，並在指標觸發時發 LINE 通知，提醒時買賣股票
...
分析程式 parse_stock.py:
<img src="./doc/screenshot_parse_stock.png" width=1104/>   

LINE 通知:  
<img src="./doc/screenshot_linebot.jpg" style="zoom:30%" />   

## 如何開始

 以下步驟於 Ubuntu 18.04 測試  
...
```
 ...
 1. 此套件使用 matplotlib 繪圖，需設定中文字形，不然圖片的中文字無法顯示  
     (1) 編輯 matplotlib 設定檔
```sh  
...
```
     ...
     <img src="./doc/matplotlib_setting.png" style="zoom:80%" />   
...
```        
確保 /agastock/out/ 目錄下有產生 stock_summary_tw.csv 及 stock_summary_us.csv 
...
```
    ...
    /home/ubuntu/agastock 須置換成實際路徑
```python    
    #agastock bootup  
    @reboot ubuntu  cd /home/ubuntu/agastock/ && /usr/bin/python3 web_root.py  
    @reboot ubuntu  cd /home/ubuntu/agastock/ && /usr/bin/python3 parse_stock.py -tw  
    @reboot ubuntu  cd /home/ubuntu/agastock/ && /usr/bin/python3 parse_stock.py -us  

    #agastock tw
    */10   9-15   * * 1-5  ubuntu cd /home/ubuntu/agastock/ && /usr/bin/python3 parse_stock.py -tw  
    30     */2    * * *    ubuntu cd /home/ubuntu/agastock/ && /usr/bin/python3 parse_stock.py -tw  

    #agastock us
    */10   21-23  * * 1-5  ubuntu cd /home/ubuntu/agastock/ && /usr/bin/python3 parse_stock.py -us  
    */10   0-4    * * 2-6  ubuntu cd /home/ubuntu/agastock/ && /usr/bin/python3 parse_stock.py -us  
    30     */2    * * *    ubuntu cd /home/ubuntu/agastock/ && /usr/bin/python3 parse_stock.py -us      
```
...
1. 重開機後，web_root.py 應該自動執行。再次檢查 http://127.0.0.1:8080/ 是否正常顯示台股美股。以下為網址範例：
...
config.py 包含所有設定，常變更的有：
 ...
 - flask web server:  
   ...
   - WEB_SERVER_PORT 可更改 http 註冊的 port。若設定 80，在 ubuntu 需執行下命令</font>  
```sh sudo 
...
```
   應該要看到
   > <small>ubuntu@ip-172-31-19-156:~$ grep CRON /var/log/syslog  
   ...
   > Sep  4 23:20:15 ip-172-31-19-156 CRON[501]: (ubuntu) CMD ( cd /home/ubuntu/agastock/ && /usr/bin/python3 web_root.py)  
   > Sep  4 23:20:15 ip-172-31-19-156 CRON[494]: (ubuntu) CMD ( cd /home/ubuntu/agastock/ && /usr/bin/python3 parse_stock.py -us)  
   > Sep  4 23:20:15 ip-172-31-19-156 CRON[500]: (ubuntu) CMD ( cd /home/ubuntu/agastock/ && /usr/bin/python3 parse_stock.py -tw)  
   ...
   > Sep  4 23:20:42 ip-172-31-19-156 CRON[452]: (CRON) info (No MTA installed, discarding output)</small>  
...
```sh  
rm -f /home/ubuntu/agastock/out/*  /home/ubuntu/agastock/static/logs/* \
      /home/ubuntu/agastock/static/img/* /home/ubuntu/agastock/__pycache__/* \
...
## 各套件使用限制
 - FinMind： 未輸入TW_FM_TOKEN每小時300次，免費會員每小時600次，付費Backer提高到每小時1600次，https://finmindtrade.com/analysis/#/Sponsor/sponsor
 - 證交所：每 5 秒鐘 3 個 request，超過的話會被 ban 掉，據說每次request間隔1秒即可，https://github.com/mlouielu/twstock
 - Google Trend： 據說每小時400次，https://github.com/GeneralMills/pytrends/issues/202
 - LINE bot ：免費帳號每月可發送500則訊息，https://tw.linebiz.com/service/account-solutions/line-official-account/
 - Yahoo Finance： 據說每小時 2000次 request，https://towardsdatascience.com/free-stock-data-for-python-using-yahoo-finance-api-9dafd96cad2e
...
## 軟體設計筆記
 - parse_stock.py 為股票分析主程式，使用 StockUs 和 StockTwn 取得美股台股資料，將不同股票(ticker)儲存於 _data[ticker]。各個 _data[ticker] 一開始為無元素 dict， 建立步驟為：
   1. queue_xxx() 根據參數 init_vars[col1, col2...]， 將 _data[ticker] 加入新的 column，成為  _data[ticker][col1], _data[ticker][col2],..., 並填入初始值 float('nan')
   
   1. 初始值選用 float('nan') 的優點是，它可被印出 "%f"%float('nan') 也可做數學運算 float('nan') + 1，結果都是 nan 但不會觸發 exception，可簡化錯誤處理. 但不能使用 %d 列印, '%d'%float('nan') 會造成 exception

   1. queue_xxx() 從網路下載股票資料，將有下載到的資料填入 _data[ticker][col]. 無資料 column 還是 float('nan')，例如新股91app尚無年度EPS
 ...
 - queue_xxx() 股票分析函式
   ...
   - 使用 QueryHandler 的 queue_xxx() 需符合以下介面，分為兩類:
     - query_XX(self) <font color=#008000> #@QueryHandler_NoLoop </font>
     - query_OO(self, ticker, tdata, prev_tdata)  <font color=#008000> #@QueryHandler_ForLoop, @QueryHandler_ThreadLoop </font>
 ...
 - 命名規則
     - _XXX，在父類別 StockBase 和子類別 StockUs/StockTwn 之間共用，外界不該使用
 
## Q&A
  1. 如何增加股票？   
     - 請修改 config.py 的 US_TICKER_LIST (美股) 及 TW_TICKET_LIST (台股)　<br><br>
...
```        
  ...
  1. 此套件如何儲存資料，有用到資料庫嗎？    
     - 此套件未使用資料庫，只用 csv 檔案儲存。parse_stock.py 儲存兩類檔案：  
       (1) stock_summary_xx.csv 儲存所有股票資料，提供 web_root.py 顯示於網頁  
       (2) line_notify_xx.csv 儲存 line 通知紀錄，避免同一筆通知重複傳送。每種通知皆有過期天數，例如台股處置股設定 14 天過期，符合證交所慣例<br><br>  
    
  1. 如何避免查詢太密集被網站ban？     
     - 此套件設有緩存機制，支援的 queue_xxx() 將資料儲存於 stock_summary_xx.csv ，在參數 expire_hours 到期前，會沿用 csv 資料而不會重新查詢，避免被 ban  
     - 支援緩存的函式有：      
```python  
 query_google_trend() #有效期預設為 GTREAD_EXPIRE_HOURS=4 小時  
 query_finance() #有效期預設為 FINANCE_EXPIRE_HOURS=3*24，三天  
```   
     - 不支援緩存的函式有：
```python  
 query_price(), _query_price_yfinance() #查詢股價需即時，不緩存。收盤後不常執行應該不會被 ban  
 query_bband() #需使用即時股價計算，配合 query_price() 不緩存    
 query_info()  #hard code 不需網路下載，無需緩存  
 台股 _query_punish_stock() #台股證交所只要隔一秒讀取就不會被 ban  
```
 
  1. 為何股價圖有時候有斷層，或是一段時間交易量異常高？  
     - 請見[已知問題](#已知問題)，此為 Yahoo Finance 的 bug<br><br>
 
  1. LINE 通知需要收費嗎？
     - 目前免費，一個月可傳送 400 則訊息。欲開通 LINE 通知服務，請在 https://developers.line.biz/zh-hant/ 註冊帳號，並將 LINEBOT_ID 及 LINEBOT_TOKEN 填入 config.py  
 
## 已知問題
 - Yahoo Finance 讀取台股，有時股價全部 NaN，並且股價日期為週末
 - Yahoo Finance 有時候部分日期 NaN，並且回去幾天Volume都爆量一百倍，可開啟YFINANCE_FIX_HIGH_VOLUME啟用修正機制
